...
export function MobileBottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-6 border-t border-hairline bg-white/95 backdrop-blur pb-[env(safe-area-inset-bottom)] lg:hidden">
      {links.map((link) => {
        ...
        return (
          <Link
            ...
            className={`flex flex-col items-center justify-center gap-1 py-2.5 text-[10px] font-medium transition-colors ${
              active ? 'text-brand-purple' : 'text-ink-500'
            }`}
          >
